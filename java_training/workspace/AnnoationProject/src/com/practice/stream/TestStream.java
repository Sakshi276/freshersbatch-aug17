...
public class TestStream {
	...
	public static void main(String[] args) {
		...
		/* Optional<Transaction> opTransaction = transactions.stream() 
				.filter(t -> t.getTrader().getCity()=="Indore") 
				.findAny(); 
		System.out.println(opTransaction); */
		
		/* List<Integer> transactionValuesFromDelhi=transactions.stream()
				.filter(t -> t.getTrader().getCity()=="Delhi")
				.map(Transaction::getValue)
				.collect(Collectors.toList());
		System.out.println(transactionValuesFromDelhi); */
		...
		Optional<Transaction> minValue = transactions.stream()
				...
				.collect(minBy(Comparator.comparing(Transaction::getValue))); 
